...
public class JavaSdkGenerator extends AbstractJavaCodegen implements NameService {
    ...
    public void processOpts() {
        ...
        service = KeywordsUtil.getKeyword(camelize(openAPI.getInfo().getTitle(), CamelizeOption.LOWERCASE_FIRST_LETTER));
        subService = camelize(openAPI.getInfo().getDescription(), CamelizeOption.LOWERCASE_FIRST_LETTER);
        apiPackage = String.format("com.kucoin.universal.sdk.generate.%s.%s", service, subService);
        modelPackage = String.format("com.kucoin.universal.sdk.generate.%s.%s", service, subService);
