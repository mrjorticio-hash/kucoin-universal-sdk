...
public class AddOrderReq implements Request {
  ...
  /**
   * A mark to force-hold the funds for an order, even though it's an order to reduce the position
   * size. This helps the order stay on the order book and not get canceled when the position size
   * changes. Set to false by default. The system will force-freeze a certain amount of funds for
   * this order, including orders whose direction is opposite to the current positions. This feature
   * is to ensure that the order won’t be canceled by the matching engine in such a way that not
   * enough funds are frozen for the order.
   */
   ...
   * strategies: CN, CO, CB. DC not currently supported.
   */
  @JsonProperty("stp")
  private StpEnum stp;
   ...
   * postOnly is true, choosing hidden or iceberg is not allowed. The post-only flag ensures that
   * the trader always pays the maker fee and provides liquidity to the order book. If any part of
   * the order is going to pay taker fees, the order will be fully rejected.
   */
   ...
   * book. When iceberg is chosen, choosing postOnly is not allowed.
   */
