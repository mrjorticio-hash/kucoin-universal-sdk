...
public class AddOrderReq implements Request {
  ...
  /**
   * When **type** is market, select one out of two: size or funds When placing a market order, the
   * funds field refers to the funds for the priced asset (the asset name written latter) of the
   * trading pair. The funds must be based on the quoteIncrement of the trading pair. The
   * quoteIncrement represents the precision of the trading pair. The funds value for an order must
   * be a multiple of quoteIncrement and must be between quoteMinSize and quoteMaxSize.
   */
  @JsonProperty("funds")
