...
public class AddStopOrderReq implements Request {
  /**
  ...
  /**
   * Specify price for order, not need for market order. When placing a limit order, the price must
   * be based on priceIncrement for the trading pair. The price increment (priceIncrement) is the
   * price precision for the trading pair. For example, for the BTC-USDT trading pair, the
   * priceIncrement is 0.00001000. So the price for your orders cannot be less than 0.00001000 and
   * must be a multiple of priceIncrement. Otherwise, the order will return an invalid
   ...
   * trading. Required for limit orders.
   */
  ...
  /**
   * passive order labels, this is disabled when the order timing strategy is IOC or FOK if **type**
   * is limit.
   */
